...
class SupplierLedgerActivity : AppCompatActivity() {

    private lateinit var transactionContainer: LinearLayout
    private lateinit var emptyState:           LinearLayout
    private lateinit var tvSupplierName:       TextView
    private lateinit var tvSupplierAddress:    TextView
    private lateinit var tvSupplierInitial:    TextView
    private lateinit var tvClosingBalance:     TextView

    private var supplierUid    = ""
    private var closingBalance = 0.0

    private var supplierName     = ""
    private var supplierFirmName = ""
    private var supplierPhone    = ""
    private var supplierAddress  = ""
    private var supplierGstin    = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supplierUid = intent.getStringExtra(EXTRA_SUPPLIER_UID) ?: run { finish(); return }
    ...
    private fun bindViews() {
        ...
        tvSupplierName       = findViewById(R.id.customerName)
        tvSupplierAddress    = findViewById(R.id.customerAddress)
        tvSupplierInitial    = findViewById(R.id.customerInitial)
    ...
    private fun loadSupplierDetails() {
        db.collection("suppliers")
            .document(supplierUid)
            .get()
            .addOnSuccessListener { doc ->
                supplierName     = doc.getString("name")     ?: ""
                supplierFirmName = doc.getString("firmName") ?: ""
                supplierPhone    = doc.getString("phone")    ?: ""
                supplierAddress  = doc.getString("address")  ?: ""
                supplierGstin    = doc.getString("gstin")    ?: ""
                closingBalance   = doc.getDouble("closingBalance") ?: 0.0

                tvSupplierName.text    = supplierName
                tvSupplierAddress.text = supplierAddress
                tvSupplierInitial.text = supplierName.firstOrNull()?.uppercaseChar()?.toString() ?: "?"
            ...
            .addOnFailureListener {
                Toast.makeText(this, "Failed to load supplier", Toast.LENGTH_SHORT).show()
            }
    }
    ...
    private fun buildPdf(docs: List<com.google.firebase.firestore.DocumentSnapshot>): File {
        ...
        canvas.drawText("Supplier Ledger", margin, 38f, paint)
        ...
        paint.textSize       = 15f
        paint.isFakeBoldText = true
        canvas.drawText(supplierName, margin + 16f, y + 26f, paint)

        val details = listOf(
            "Firm"    to supplierFirmName,
            "Phone"   to supplierPhone,
            "Address" to supplierAddress,
            "GSTIN"   to supplierGstin
        )
        var detailY = y + 46f
        ...
        val col4 = pageWidth - margin - 65f
        val col5 = pageWidth - margin
        ...
        for (doc in docs) {
            ...
            val description = doc.getString("description") ?: ""
            val date        = doc.getString("date")        ?: ""
            val billId      = doc.getString("purchaseId")  ?: doc.getString("salesId") ?: ""
        ...
        val file = File(cacheDir, "ledger_${supplierName.replace(" ", "_")}.pdf")
    ...
    private fun sharePdf(file: File) {
        ...
        val intent = Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "Ledger — $supplierName")
    ...
    companion object {
        const val EXTRA_SUPPLIER_UID = "SUPPLIER_UID"
        fun newIntent(context: Context, supplierUid: String) =
            Intent(context, SupplierLedgerActivity::class.java).apply {
                putExtra(EXTRA_SUPPLIER_UID, supplierUid)
            }
    }
}