package com.example.cashbook.ledger.bank
...
class BankLedgerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_bank_ledger)
    ...
    private fun loadFirmBalance() {
        db.collection("firm").document("main").get()
            .addOnSuccessListener { doc ->
                ...
                tvClosingBalance.text = formatAmount(bank)
            }
    }

    private fun listenToTransactions() {
        listenerRegistration = db.collection("bank")
            .document("main")
    ...
    private fun verifyBalance(computedBalance: Double) {
        db.collection("firm").document("main").get()
            .addOnSuccessListener { doc ->
                val firmBank = doc.getDouble("bank") ?: 0.0
                if (Math.abs(computedBalance - firmBank) > 0.01) {
                    Toast.makeText(
                        this,
                        "⚠ Balance mismatch! Computed: ${formatAmount(computedBalance)} | Firm: ${formatAmount(firmBank)}",
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
    ...
    companion object {
        fun newIntent(context: Context) =
            Intent(context, BankLedgerActivity::class.java)
    }
}