...
class SalesReportActivity : AppCompatActivity() {
    ...
    private val rows = mutableListOf<SalesBillRow>()
    ...
    private fun setThisYear() {
        ...
        highlightPreset(btnThisYear); loadReport()
    }
    ...
    private fun highlightPreset(active: MaterialButton) {
        listOf(btnToday, btnThisMonth, btnThisYear, btnCustomRange).forEach {
            it.backgroundTintList = android.content.res.ColorStateList.valueOf(
                if (it == active) Color.parseColor("#1B5E20") else Color.parseColor("#2A2A3E")
            )
        }
    }
    ...
    private fun loadReport() {
        ...
        // Read directly from transactions collection, filter by sales type
        db.collection("transactions")
            ...
            .addOnSuccessListener { snapshot ->
                if (snapshot.isEmpty) {
                    ...
                    tvTotalSales.text      = formatAmount(0.0)
                        ...
                        .addOnSuccessListener { billDoc ->
                            ...
                            db.collection("customers").document(customerId).get()
                                .addOnSuccessListener { custDoc ->
                                    val firm = custDoc.getString("firmName")
                                        ?: custDoc.getString("name") ?: ""
                                    rows.add(SalesBillRow(salesId, date, firm, amount, 0.0))
                                    if (--pending == 0) finishLoad()
                                }
    ...
    private fun finishLoad() {
        // Sort by date then compute running total
        rows.sortBy { it.date }
        var running = 0.0
        ...
        val totalSales = rows.sumOf { it.billAmount }

        progressBar.visibility     = View.GONE
        reportContainer.visibility = View.VISIBLE
        btnExport.isEnabled        = true
        tvTotalSales.text          = formatAmount(totalSales)
    ...
    private fun renderReport() {
        ...
        addDivider(Color.parseColor("#1B5E20"), 2)
        addSummaryRow("Total Bills",  "${rows.size}",           Color.WHITE)
        addSummaryRow("Total Sales",  formatAmount(totalSales), Color.parseColor("#A5D6A7"))
    }
    ...
    private fun addBillRow(row: SalesBillRow, index: Int) {
        ...
        listOf(
            ...
            r.addView(TextView(this).apply {
                ...
                textSize  = 10f
                setTextColor(when (i) {
                    3    -> Color.parseColor("#A5D6A7")
                    4    -> Color.parseColor("#90CAF9")
        ...
        addDivider(Color.parseColor("#2A2A3E"), 1)
    }
    ...
    private fun generateAndSharePdf() {
        ...
        startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "Sales Report — ${tvDateRange.text}")
    ...
    private fun buildPdf(): File {
        ...
        canvas.drawText("Sales Report", margin, 40f, paint)
        ...
        paintR.color = Color.parseColor("#A5D6A7"); paintR.textSize = 11f
        ...
        listOf(
            ...
            Triple("Total Sales",  formatAmount(totalSales), margin + 250f)
        ).forEach { (label, value, x) ->
            paint.color = Color.parseColor("#33691E"); paint.textSize = 9f; paint.isFakeBoldText = false
            canvas.drawText(label, x, y + 18f, paint)
            paint.color = Color.parseColor("#1B5E20"); paint.textSize = 13f; paint.isFakeBoldText = true
            canvas.drawText(value, x, y + 38f, paint)
        }
        y += 66f

        // ── Column positions ──────────────────────────────────────
        val c1 = margin + 2f       // Date
        val c2 = margin + 76f      // Bill ID
        val c3 = margin + 190f     // Firm
        val c4 = margin + 360f     // Amount (right)
        val c5 = pageWidth - margin // Running total (right)
        ...
        fun drawTableHeader() {
            ...
            paintR.color = Color.WHITE; paintR.textSize = 9f; paintR.isFakeBoldText = true
        ...
        rows.forEachIndexed { i, row ->
            checkBreak(26f)
            if (i % 2 == 0) {
                paint.color = Color.parseColor("#F9FBF9")
        ...
        checkBreak(50f); y += 6f
        listOf(
            Triple("Total Bills",  "${rows.size} bills",    Color.parseColor("#1B5E20")),
            Triple("Total Sales",  formatAmount(totalSales), Color.parseColor("#1B5E20"))
        ...
        canvas.drawText("CashBook Sales Report  |  ${tvDateRange.text}  |  $dateStr", margin, pageHeight - 14f, paint)

        pdf.finishPage(page)
        val file = File(cacheDir, "sales_report_${System.currentTimeMillis()}.pdf")
    ...
    companion object {
        fun newIntent(context: Context) = Intent(context, SalesReportActivity::class.java)
    }
}